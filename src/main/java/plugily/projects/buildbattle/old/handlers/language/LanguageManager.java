...
import plugily.projects.buildbattle.old.utils.services.locale.Locale;
import plugily.projects.buildbattle.old.utils.services.locale.LocaleRegistry;
import plugily.projects.buildbattle.old.utils.services.locale.LocaleService;
  ...
  private static List<String> getStrings(String path) {
    ...
    if(!language.isSet(path)) {
      ...
      if(!language.isSet(path)) {
        ...
        Debugger.sendConsoleMsg("&c[Build Battle] Please regenerate your old_language.yml file! If error still occurs report it to the developer on discord!");
