      ...
      public void execute(CommandSender sender, String[] args) {
        ...
        if(arena.getArenaState() == ArenaState.IN_GAME && registry.getPlugin().getConfigPreferences().getTimer(ConfigPreferences.TimerType.BUILD, arena) - arena.getTimer() <= 20) {
          ...
          if(arena.getArenaState() == ArenaState.STARTING) {
            sender.sendMessage(registry.getPlugin().getChatManager().getPrefix() + registry.getPlugin().getChatManager().colorMessage("Commands.Wait-For-Start")); // NOT PLAYING
          } else {
            sender.sendMessage(registry.getPlugin().getChatManager().getPrefix() + registry.getPlugin().getChatManager().colorMessage("Commands.BaseArena-Started"));
          }
        }
      }
    });
