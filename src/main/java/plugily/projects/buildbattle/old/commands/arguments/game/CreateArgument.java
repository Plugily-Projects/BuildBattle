  ...
  public CreateArgument(ArgumentsRegistry registry) {
    ...
    registry.mapArgument("buildbattle", new LabeledCommandArgument("create", "buildbattle.admin.create", CommandArgument.ExecutorType.PLAYER,
      ...
      public void execute(CommandSender sender, String[] args) {
        if(args.length == 1) {
          sender.sendMessage(registry.getPlugin().getChatManager().colorMessage("Commands.Type-BaseArena-Name"));
          return;
        }
        Player player = (Player) sender;
        for(BaseArena arena : ArenaRegistry.getArenas()) {
          if(arena.getID().equalsIgnoreCase(args[1])) {
            player.sendMessage(ChatColor.DARK_RED + "BaseArena with that ID already exists!");
        ...
        if(ConfigUtils.getConfig(registry.getPlugin(), "arenas").contains("instances." + args[1])) {
          player.sendMessage(ChatColor.DARK_RED + "Instance/BaseArena already exists! Use another ID or delete it first!");
        } else {
