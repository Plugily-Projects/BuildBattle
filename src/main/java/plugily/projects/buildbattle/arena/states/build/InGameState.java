      ...
      case BUILD_TIME:
        ...
        if(arena.getTimer() <= 0) {
          ...
          for(Player player : pluginArena.getPlayersLeft()) {
            ...
            Plot plotFromPlayer = pluginArena.getPlotFromPlayer(player);
            if(!pluginArena.getQueue().contains(plotFromPlayer)) {
              pluginArena.getQueue().add(plotFromPlayer);
            }
  ...
  private void calculateEndResults(BuildArena pluginArena) {
    ...
    for(Plot buildPlot : pluginArena.getPlotManager().getPlots()) {
      for(Map.Entry<Integer, List<Player>> map : new HashMap<>(pluginArena.getTopList()).entrySet()) {
        ...
        Plot plot = pluginArena.getPlotManager().getPlot(map.getValue().get(0));
        if(plot == null) {
    ...
    if(!pluginArena.getQueue().isEmpty()) {
      ...
      for(Player p : pluginArena.getPlayers()) {
        ...
        if(getPlugin().getConfigPreferences().getOption("HIDE_PLOT_OWNER")) {
          ...
          new TitleBuilder("IN_GAME_MESSAGES_PLOT_VOTING_PLOT_OWNER_TITLE").asKey().arena(pluginArena).value(formattedMembers).player(p).sendPlayer();
          new MessageBuilder("IN_GAME_MESSAGES_PLOT_VOTING_PLOT_OWNER_NEXT").asKey().arena(pluginArena).value(formattedMembers).player(p).sendPlayer();
        }
      }
