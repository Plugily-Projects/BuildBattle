...
public class Main extends PluginMain {
  ...
  public void registerPlaceholders() {
    ...
    for(int i = 0; i <= 32; i++) {
      final int number = i;
      getPlaceholderManager().registerPlaceholder(new Placeholder("place_member_" + i, Placeholder.PlaceholderType.ARENA, Placeholder.PlaceholderExecutor.ALL) {
        @Override
        public String getValue(Player player, PluginArena arena) {
      ...
      getPlaceholderManager().registerPlaceholder(new Placeholder("place_points_" + i, Placeholder.PlaceholderType.ARENA, Placeholder.PlaceholderExecutor.ALL) {
        @Override
        public String getValue(Player player, PluginArena arena) {
          return getPlace(arena);
        }
      ...
      private String getSummary(PluginArena arena, Player player) {
        ...
        if(pluginArena instanceof BuildArena) {
          ...
          for(int a = 0; a < playerList.size(); a++) {
            if(playerList.get(a).contains(player)) {
              return new MessageBuilder("IN_GAME_MESSAGES_GAME_END_PLACEHOLDERS_OWN").asKey().integer(a).arena(pluginArena).build();
            }
          }
        }
        if(pluginArena instanceof GuessArena) {
          ...
          int index = new ArrayList<>(((GuessArena) pluginArena).getPlayersPoints().keySet()).indexOf(player);
          if(index != -1) {
            return new MessageBuilder("IN_GAME_MESSAGES_GAME_END_PLACEHOLDERS_OWN").asKey().integer(index).arena(pluginArena).build();
          }
        }
